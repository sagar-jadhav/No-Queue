...
import { add, userID , login} from '../lib/utils'
...
const Login = function ({ navigation }) {
  const clearItem = {owner_id: '',password: ''}
  ...
  const sendItem = () => {
    ...
    login(payload)
      .then(() => {
        Alert.alert('Yay!', 'You have been logged in.', [{text: 'OK', onPress: () => {navigation.navigate('Map');}}]);
        setItem({ ...clearItem });
      })
      .catch(err => {
